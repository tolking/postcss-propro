...
function colorRegExp(str) {
    return /(^#[0-9a-fA-F]{6}$)|(^#[0-9a-fA-F]{3}$)/i.test(str) ||
    /^[rR][gG][Bb]/.test(str) ||
    /^[hH][sS][lL]/.test(str);
}
function makeValue(values, opts) {
    ...
    if (colorRegExp(values[1])) {
        color = values[1];
        time = values[2] || '1s';
    } else {
        width = values[1];
        if (colorRegExp(values[2])) {
            color = values[2];
            time = values[3] || '1s';
        } else {
            height = values[2];
            if (colorRegExp(values[3])) {
                color = values[3];
                time = values[4] || '1s';
            } else {
